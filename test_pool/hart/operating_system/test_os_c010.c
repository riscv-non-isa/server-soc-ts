...
set_pmu_overflow()
{
  ...
  val_hart_reg_write(PMINTENCLR_EL1, 0xFFFFFFFF);
  val_hart_reg_write(PMOVSCLR_EL0, 0xFFFFFFFF);

  //Sequence to generate PMUIRQ
  pmcr = val_hart_reg_read(PMCR_EL0);
  val_hart_reg_write(PMCR_EL0, pmcr|0x1);

  val_hart_reg_write(PMINTENSET_EL1, 0x1);
  val_hart_reg_write(PMOVSSET_EL0, 0x1);
...
isr()
{
  ...
  val_hart_reg_write(PMOVSCLR_EL0, 0x1);
...
payload()
{
  ...
  int_id = val_hart_get_pmu_gsiv(index);
  if (int_id == 0) {
...
**/
uint32_t
os_c010_entry(uint32_t num_hart)
{
